...
const meanScore = (...arrays) => {
  const flatArray = [];
  arrays.forEach((element) => flatArray.push(...element));

  if (flatArray.some((innerAarray) => typeof innerAarray !== 'number')) {
    console.error('All function arguments should be numbers!');
    return;
  }

  const result = (flatArray.reduce((acc, element) => acc += element) / flatArray.length).toFixed(2);
  return result;
};
