...
const isElementInArray = (searchElement, inputArray) => {
  if (inputArray.every((element) => element && typeof element === 'object')) {
    const stringifiedArr = inputArray.map((element) => JSON.stringify(element));
    const stringifiedElement = JSON.stringify(searchElement);
    if (stringifiedArr.includes(stringifiedElement)) {
      return true;
    }
  }
