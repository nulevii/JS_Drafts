...
const quantitiesByCategories = (products) => products.reduce(
  (ObjWithElements, element) => {
    const { quantity, category } = element;
    if (!ObjWithElements[category]) { ObjWithElements[category] = 0; }
    ObjWithElements[category] += quantity;
    return ObjWithElements;
  },
  {},
);
const inputProducts = [
